...
class DiamondCollector(gym.Env):
    ...
    def step(self, action):
        ...
        if action[2] >=0:
            attack_command = 'attack 1'
        else:
            attack_command = 'attack 0'
        
        command = attack_command
        if command != 'attack 1' or self.allow_break_action:
            if self.allow_break_action command == 'attack 1':
        ...
        for i in range(-49, 50):
            for j in range(-49, 50):
                if random.random()<= self.reward_density:
                    my_xml = my_xml + "<DrawBlock x= '" + str(i) + "' y ='2' z = '" + str(j) + "' type = 'diamond_ore' />"
        for i in range(-49, 50):
            for j in range(-49, 50):
                if random.random()<= self.penalty_density:
                    lava_xml = lava_xml + "<DrawBlock x= '" + str(i) + "' y ='1' z = '" + str(j) + "' type = 'lava' />"
        #my_xml = "<DrawBlock x= '3' y ='3' z = '3' type = 'diamond_ore' /> <DrawBlock x= '3' y ='2' z = '3' type = 'diamond_ore' />"
                ...
                <Mission xmlns="http://ProjectMalmo.microsoft.com" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
                    ...
                    <AgentSection mode="Survival">
                        ...
                        <AgentHandlers>
                            ...
                            <AgentQuitFromReachingCommandQuota total="'''+str(round(self.max_episode_steps/3))+'''" />
